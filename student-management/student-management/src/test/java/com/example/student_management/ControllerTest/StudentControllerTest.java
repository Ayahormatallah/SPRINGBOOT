package com.example.student_management.ControllerTest;
...
class StudentControllerTest {
    ...
    void testDeleteStudent() {
        // Simuler la suppression d'un étudiant qui existe
        ...
        // Vérifier que la réponse indique bien une suppression réussie
        assertEquals(HttpStatus.NO_CONTENT, response.getStatusCode());

        // Simuler la suppression d'un étudiant qui n'existe pas
        when(studentService.delete(2)).thenReturn(false);

        // Appeler le contrôleur pour tenter de supprimer un étudiant inexistant
        ResponseEntity<Void> responseFail = studentController.delete(2);

        // Vérifier que la réponse indique bien une erreur NOT_FOUND
        assertEquals(HttpStatus.NOT_FOUND, responseFail.getStatusCode());
    }
    ...
    @Test
    void testFindByYear() {
        // Simuler la récupération du nombre d'étudiants par année
        when(studentService.findStudentsByYear()).thenReturn(Arrays.asList());
        ...
        // Vérifier que la réponse est bien un objet de type List et que la collection est remplie
        assertNotNull(response.getBody());
        assertFalse(response.getBody().isEmpty());
        assertEquals(3, response.getBody().size());  // Il y a 3 éléments dans le retour simulé
        assertEquals(HttpStatus.OK, response.getStatusCode());
    }
}
